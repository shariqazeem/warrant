...
describe("confirmOpenings", () => {
  ...
  it("makes the claim the indexer makes before a grant becomes a row", () => {
    ...
    // The indexer builds the same claim inline. Read it, so the two cannot drift apart.
    const indexer = readFileSync("lib/indexer.ts", "utf8").replace(/\s+/g, " ");
    expect(indexer).toContain(
      "const opened = await backed(found, address, (l) => ({ payer: l.args.payer!, " +
        "recipient: l.args.beneficiary!, asset: l.args.asset!, stable: l.args.stableCost!, " +
        "cash: 0n, }));",
    );
  });
});
