 ...
 * Nothing here touches mainnet. Each grant buys its stock through a real OKX DEX route (the
 * aggregator's calldata, as scripts/prove-grant-fork.ts uses it), executed against the fork's
 * copy of mainnet liquidity, so it needs the OKX API credentials in .env.local. The payer's
 * USD₮0 is written into the fork's storage. The amounts exist only on this machine and must
 * never be shown as real.
 *
 *   anvil --fork-url https://rpc.xlayer.tech --port 8547 --silent &
 *   FORK_URL=http://127.0.0.1:8547 npx tsx scripts/fork-scenes.ts
 */
import {erc20Abi, keccak256, parseEventLogs, toHex, type Address, type Hex} from "viem";
...
import {rememberReason, rememberRoute} from "../lib/db";
import {routeTokens} from "../lib/grant-terms";
...
/** A recipient that exists only on the fork, from a fixed test key so it can connect there too. */
export const FORK_RECIPIENT_KEY = keccak256(toHex("warrant fork recipient"));
const RECIPIENT = privateKeyToAccount(FORK_RECIPIENT_KEY).address;
const DAY = 86_400;
...
async function main() {
  ...
  type Scene = {label: string; asset: typeof spyx; usd: bigint; start: number; cliff: number; duration: number; note: string};
  const scenes: Scene[] = [
    {label: "sealed, mid-schedule", asset: spyx, usd: 1_000_000_000n, start: now - 400 * DAY, cliff: 182 * DAY, duration: 730 * DAY, note: "Founding engineer, two-year grant"},
    {label: "revocable, before the cliff", asset: nvdax, usd: 500_000_000n, start: now, cliff: 3600, duration: 7200, note: "Launch bonus"},
    {label: "cancelled", asset: spyx, usd: 300_000_000n, start: now - 120 * DAY, cliff: 30 * DAY, duration: 365 * DAY, note: "Contractor retention"},
  ];
  ...
  for (const s of scenes) {
    ...
    const route = await swap({
      from: STABLE.address,
      to: s.asset.address,
      amount: s.usd.toString(),
      slippagePercent: "1",
      userWalletAddress: ESCROW,
      receiver: ESCROW,
    });
    if (!route.ok) throw new Error(`${s.label}: no route (${route.why})`);
    const leg = route.value[0]!;
    const minOut = BigInt(leg.tx.minReceiveAmount ?? "0");
    if (minOut === 0n) throw new Error(`${s.label}: the route stated no minimum`);
    const hops = routeTokens(
      leg.routerResult.dexRouterList ?? [],
      {address: STABLE.address, symbol: "USD₮0"},
      {address: s.asset.address, symbol: s.asset.symbol},
    );
    ...
    const tx = await client.writeContract({
      ...
      args: [{
        ...
        tipBps: 50,
        reasonHash: hash,
        routerCalldata: leg.tx.data as Hex,
      }],
    });
    ...
    rememberRoute(tx, hops);
    console.log(`grant ${ev.args.id} (${s.label}) opened in ${tx}; route: ${hops.join(" > ") || "(unnamed)"}`);
  }
