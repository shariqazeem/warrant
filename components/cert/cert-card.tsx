...
import {OG_CERT as C} from "@/lib/og-theme";
import {vestedShares} from "@/lib/vesting";

/**
 * THE CERTIFICATE, AS A SHARE CARD: the landscape certificate drawn for Satori (next/og) at
 * 1200×630 on the vault. Same seed, same fields, same seal as the page; inline styles and
 * literal colours only, because Satori has no stylesheet; no animation and no ticking figure.
 */
...
export const UI = "Hanken Grotesk";
export const MONO = "JetBrains Mono";

export function CertificateCard({d, escrow, now}: {d: CertificateData; escrow: string; now: number}) {
  const no = certNumber(d.id);
  const g = guilloche(
    certificateSeed(xLayer.id, escrow, d.id),
    LANDSCAPE.W,
    LANDSCAPE.H,
    LANDSCAPE.rx,
    LANDSCAPE.ry,
        ...
        <div style={{position: "relative", display: "flex", width: W, height: H, background: C.bond, borderRadius: 3}}>
          ...
          >
            <div style={{display: "flex", justifyContent: "space-between", alignItems: "baseline"}}>
              ...
              <div style={{display: "flex", fontFamily: DISPLAY, fontWeight: 500, fontSize: px(17)}}>{`No. ${no}`}</div>
            </div>
          ...
          <div style={{position: "absolute", right: px(30), bottom: px(24), width: px(118), height: px(118), display: "flex"}}>
            ...
            {seal === "sealed" ? (
              <div style={{position: "absolute", left: 0, right: 0, top: px(72), display: "flex", flexDirection: "column", alignItems: "center"}}>
                ...
                <div style={{display: "flex", fontFamily: DISPLAY, fontWeight: 500, fontSize: px(8.5), color: C.bond, opacity: 0.85, marginTop: px(1)}}>{`No. ${no}`}</div>
              </div>
            ) : (
